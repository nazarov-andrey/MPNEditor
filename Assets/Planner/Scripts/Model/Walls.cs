...
namespace Planner.Model
{
    ...
    public class WallSegmentBounds
    {
        public readonly Line Inner;
        public readonly Line Outer;

        public WallSegmentBounds (Line inner, Line outer)
        {
            Inner = inner;
            Outer = outer;
        }
    ...
    public class WallData
    {
        ...
        public readonly Lazy<Line[]> Segments;
        public readonly Lazy<WallSegmentBounds[]> Bounds;
            ...
            WidthChangeType widthChangeType)
        {
            ...
            Bounds = new Lazy<WallSegmentBounds[]> (GetBounds);
            InnerPoints = new Lazy<Vector2[]> (GetInnerPoints);
            OuterPoints = new Lazy<Vector2[]> (GetOuterPoints);
            Segments = new Lazy<Line[]> (GetSegments);
        }

        private Line[] GetSegments ()
        {
            var segments = new Line[Points.Length];
            var legnth = Points.Length;
            for (int i = 0; i < legnth; i++) {
                segments[i] = Line.Create (Points[i], Points[(i + 1) % legnth]);
            }
        ...
        private WallSegmentBounds[] GetBounds ()
        {
            var innerPoints = InnerPoints.Value;
            var outerPoints = OuterPoints.Value;
            var lines = new WallSegmentBounds[Points.Length - 1];
            for (int i = 0; i < innerPoints.Length - 1; i++) {
                lines[i] = new WallSegmentBounds (
    ...
    public class Walls
    {
        ...
        private readonly Dictionary<Vector2, PointWalls> pointWallsMap = new Dictionary<Vector2, PointWalls> ();
        private readonly List<WallData> allWalls = new List<WallData> ();
        ...
        public void RemoveWallData (WallData wallData)
        {
            var start = wallData.Points.First ();
            var end = wallData.Points.Last ();

            GetPointWalls (start).Remove (wallData);
            GetPointWalls (end).Remove (wallData);
            allWalls.Remove (wallData);
        }

        public int Count => pointWallsMap.Count;
        public IEnumerable<Vector2> Points => pointWallsMap.Keys;
        public WallData[] AllWalls => allWalls.ToArray ();
    }
