...
namespace Planner.RoomGeometry
{
    public static class WallMeshesGenerator
    {
        ...
        public enum Options
        {
            ...
            Sides = 16,
            TopDismissHeight = 32,
            Default = Top | Inner | Outer | Openings | Sides
        }
...
/*            Debug.Log (
            ...
            if (!(prevInnerLine.HasValue && startLines.Inner.Intersects (prevInnerLine.Value, out innerStart)))
                innerStart = innerPoints.First ();

            if (!(prevOuterLine.HasValue && startLines.Outer.Intersects (prevOuterLine.Value, out outerStart)))
                outerStart = outerPoints.First ();

            if (!(nextInnerLine.HasValue && endLines.Inner.Intersects (nextInnerLine.Value, out innerEnd))) {
            ...
            if (!(nextOuterLine.HasValue && endLines.Outer.Intersects (nextOuterLine.Value, out outerEnd))) {
                switch (wall.WidthChangeType) {
                    case WidthChangeType.Type1:
            ...
            Hole2DProjection[] outer2DHoles = null;
            Hole2DProjection[] inner2DHoles = null;

            if (options.HasFlag (Options.Openings)) {
            ...
            if (options.HasFlag (Options.Inner)) {
                ...
                result.Add (innerMesh);
            }

            if (options.HasFlag (Options.Outer)) {
                ...
                result.Add (outerMesh);
            }

            if (options.HasFlag (Options.Sides)) {
                ...
                result.Add (sideAMesh);
                result.Add (sideBMesh);
            }

            if (options.HasFlag (Options.Top)) {
                ...
                var y = options.HasFlag (Options.TopDismissHeight)
                    ? 0
                    : wall.Height;
                
                var topMesh = MeshGenerator.CreateMesh (
                    vertices.ConvertAll (x => new Vector3 (x.x, y, x.z)).ToArray (),
                    triangles.ToArray (),
                    false,
                    "top");
